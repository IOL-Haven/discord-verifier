...
public final class UserManager {
    ...
    private class UserPair {
        ...
        public UserPair(Snowflake _discord, String _minecraft) {
            discord = _discord;
        ...
        if(!USER_RECORD.exists()) {
            try {
                ...
                try (Writer writer = new FileWriter(USER_RECORD)) {
                    GSON.toJson(new ArrayList<UserPair>(), writer);
                }
    ...
    public Optional<String> writeUsername(Snowflake discord, String minecraft) {
        ...
        try (Reader reader = new FileReader(USER_RECORD)) {
            ...
            users.add(new UserPair(discord, minecraft));
        } catch (Exception e) {
    ...
    private static Optional<String> eraseExistingEntry(List<UserPair> users, Snowflake discord) {
        // There won't be multiple entries matching the condition in the list, so the join() functionality will never get used.
        String out = String.join(", ", users.stream().filter(pair -> pair.discord.equals(discord)).map(pair -> pair.minecraft).toList());
        users.removeIf(pair -> pair.discord.equals(discord));
    ...
    public boolean userIsVerified(GameProfile minecraftUser) {
        ...
        try (Reader reader = new FileReader(USER_RECORD)) {
            ...
            return users.stream().anyMatch(pair -> matchMinecraftUsername(minecraftUser, pair.minecraft));
        } catch (Exception e) {
